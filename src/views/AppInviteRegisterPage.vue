    ...
    <section class="join-shell">
      ...
      <article class="register-card">
        ...
        <div v-if="success" class="success-panel">
          ...
          <button class="app-login-link" type="button" @click="openAppLogin">
            {{ text.goLogin }}
          </button>
        </div>
        ...
        <form v-else @submit.prevent="submit">
          ...
          <button class="login-link" type="button" @click="openAppLogin">
            {{ text.hasAccount }}
          </button>
        </form>
      </article>
      ...
      <section id="app-download" ref="downloadCard" class="download-card">
        <div>
          <p>{{ text.getApp }}</p>
...
.login-link,.app-login-link{border:0;background:transparent;cursor:pointer}
...
.brand-row{justify-content:space-between}.brand-row>.brand-lockup{display:flex;flex-direction:row;align-items:center;gap:15px;min-width:0;padding-left:0;border-left:0}.brand-lockup>div{display:flex;flex-direction:column;padding-left:15px;border-left:1px solid #d5d8dc}
