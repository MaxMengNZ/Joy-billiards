      ...
      <header class="app-header" v-if="!isAuthPage && !isTVDisplayPage && !isSharePage">
        <div class="header-content">
          ...
          <nav class="main-nav desktop-nav">
            ...
            <router-link to="/membership" class="nav-link">💳 {{ t('nav.membership') }}</router-link>
            <router-link to="/leaderboard" class="nav-link">🏆 {{ t('nav.rankings') }}</router-link>
            <router-link to="/songs" class="nav-link">🎵 {{ t('nav.songs') }}</router-link>
            <router-link to="/battle" class="nav-link">⚔️ {{ t('nav.battle') }}</router-link>
            <router-link to="/tournaments" class="nav-link">{{ t('nav.calendar') }}</router-link>
            <router-link v-if="authStore.isAdmin" to="/players" class="nav-link">{{ t('nav.players') }}</router-link>
            <router-link v-if="authStore.isAdmin" to="/admin" class="nav-link">{{ t('nav.admin') }}</router-link>
            <router-link v-if="authStore.isAdmin" to="/tv-display" class="nav-link">📺 {{ t('nav.tvDisplay') }}</router-link>
            <router-link v-if="authStore.isAdmin" to="/songs/venue-qr" class="nav-link">🎵 {{ t('nav.songQr') }}</router-link>
            ...
            <div class="auth-nav">
              ...
              <template v-if="authStore.isAuthenticated">
                ...
                <button class="nav-link btn-logout" @click="handleLogout">{{ t('nav.logout') }}</button>
              </template>
              <template v-else>
                <router-link to="/login" class="nav-link">{{ t('nav.login') }}</router-link>
                <router-link to="/register" class="nav-link btn-register">{{ t('nav.signUp') }}</router-link>
              </template>
            </div>
          </nav>
          ...
          <nav id="mobile-nav" class="main-nav mobile-nav" :class="{ open: isMobileMenuOpen }" aria-label="Mobile navigation">
            ...
            <router-link to="/membership" class="nav-link" @click="closeMobileMenu">💳 {{ t('nav.membership') }}</router-link>
            <router-link to="/leaderboard" class="nav-link" @click="closeMobileMenu">🏆 {{ t('nav.rankings') }}</router-link>
            <router-link to="/songs" class="nav-link" @click="closeMobileMenu">🎵 {{ t('nav.songs') }}</router-link>
            <router-link to="/battle" class="nav-link" @click="closeMobileMenu">⚔️ {{ t('nav.battle') }}</router-link>
            <router-link to="/tournaments" class="nav-link" @click="closeMobileMenu">{{ t('nav.calendar') }}</router-link>
            <router-link v-if="authStore.isAdmin" to="/players" class="nav-link" @click="closeMobileMenu">{{ t('nav.players') }}</router-link>
            <router-link v-if="authStore.isAdmin" to="/admin" class="nav-link" @click="closeMobileMenu">{{ t('nav.admin') }}</router-link>
            <router-link v-if="authStore.isAdmin" to="/tv-display" class="nav-link" @click="closeMobileMenu">📺 {{ t('nav.tvDisplay') }}</router-link>
            <router-link v-if="authStore.isAdmin" to="/songs/venue-qr" class="nav-link" @click="closeMobileMenu">🎵 {{ t('nav.songQr') }}</router-link>
            ...
            <div class="auth-nav mobile-auth">
              ...
              <template v-else>
                ...
                <router-link to="/register" class="nav-link btn-register" @click="closeMobileMenu">{{ t('nav.signUp') }}</router-link>
              </template>
            </div>
          </nav>
        </div>
        ...
        <div class="footer-content">
          ...
          <div class="footer-links">
            ...
            <div class="social-media-links">
              <a href="https://www.facebook.com/profile.php?id=61578291092113" target="_blank" rel="noopener noreferrer" class="social-icon facebook" :title="t('footer.facebook')">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
              </a>
              <a href="https://www.instagram.com/joybilliardsnz" target="_blank" rel="noopener noreferrer" class="social-icon instagram" :title="t('footer.instagram')">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
              ...
              <router-link to="/wechat" class="social-icon wechat" :title="t('footer.wechat')">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
              ...
              <a href="https://xhslink.com/m/2CwhEpZgtub" target="_blank" rel="noopener noreferrer" class="social-icon xiaohongshu" :title="t('footer.xhs')">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
...
export default {
  ...
  setup() {
    ...
    const isSharePage = computed(() => {
      ...
      return isClubDomain || route.path === '/pricing' || route.path === '/membership' || route.path.startsWith('/share/') || route.path.startsWith('/app/player/') || route.path === '/app/monthly-honours' || route.path === '/app/annual-honours' || route.path === '/join'
    })
    ...
    const handleLogout = async () => {
      ...
      const userConfirmed = confirm(t('nav.logoutConfirm'))
      if (userConfirmed) {
